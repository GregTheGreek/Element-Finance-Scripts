...
import { CRV_WITHDRAWAL_TRANSACTIONS_2 } from './constants';
...
async function proposal() {
  ...
  for (let i in CRV_WITHDRAWAL_TRANSACTIONS_2) {
    const treasuryCallData = treasuryVotingInterface.encodeFunctionData('genericCall', [
        CRV_WITHDRAWAL_TRANSACTIONS_2[i].to,
        CRV_WITHDRAWAL_TRANSACTIONS_2[i].data
    ]);
